		...
		link: function(scope) {
			scope.$watch("customfield.Value", function(val) {
				...
				scope.customfield.Value = val[
					scope.case == "upper" ? "toUpperCase" : "toLowerCase"
				]();
			});
		}
	};
...
function ocselectionfield($451) {
	...
	function OCSelectionFieldLink(scope, element, attr) {
		scope.changed = function() {
			...
			scope.customfield.SelectedOptionID =
				this.item == null ? null : this.item.ID;
			if (this.item != null) this.item.Selected = true;

			if (
				this.item != null &&
				this.item.Value.indexOf("Other") > -1 &&
				this.item.ID.indexOf("_other") > -1
			) {
			...
			if (scope.change) scope.change(scope.customfield);
		};
...
function ocfilefield($parse, $resource, $451, fileReader, Security) {
	...
	function OCFileFieldLink(scope, element, attrs) {
		...
		var afterSelection = function(file) {
			...
			$resource($451.api("uploadfile"))
				.save({
					Data: file.result,
					Name: file_control.files[0].name,
					ID: scope.customfield.ID,
					SourceType: scope.customfield.SourceType,
					SourceID: scope.customfield.SourceID
				})
		...
		var updateModel = function(event) {
			...
			switch (event.target.name) {
				...
				case "upload":
					...
					scope.$apply(function() {
						fileReader
							.readAsDataUrl(event.target.files[0], scope)
							.then(afterSelection);
		...
		link: function(scope) {
			scope.$watch("customfield.Date", function(newVal) {
				...
				scope.customfield.Value = $filter("date")(
					scope.customfield.Date,
					scope.format
				);
			});
			scope.$watch("customfield.Value", function(newVal) {
				if (!newVal) return;
				scope.customfield.Date = new Date(
					getDateFromFormat(scope.customfield.Value, scope.format)
				);
			});
		}
	};
	...
	return {
		...
		link: function(scope, elm, attr, ngModel) {
			...
			var ck = CKEDITOR.replace(elm[0], {
				removeButtons:
					"Source,NumberedList,BulletedList,Outdent,Indent,Link,Unlink,Anchor"
